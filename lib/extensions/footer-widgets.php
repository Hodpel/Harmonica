...
add_action( 'widgets_init', 'harmonica_register_footer_widget_areas', 20 );
 ...
 * @since 0.3.4
 *
 * @uses harmonica_register_sidebar() Register footer widget areas.
 *
 * @return null Return early if there's no theme support.
 */
function harmonica_register_footer_widget_areas() {

	$footer_widgets = get_theme_support( 'harmonica-footer-widgets' );
	...
	while ( $counter <= $footer_widgets ) {
		...
		$defaults = array(
			...
			'name'          => sprintf( __( 'Footer %d', 'harmonica' ), $counter ),
			'description'   => sprintf( __( 'Footer %d widget area.', 'harmonica' ), $counter )
		);

		harmonica_register_sidebar( $defaults );
...
add_action( 'harmonica_before_footer', 'harmonica_footer_widget_areas' );
/**
 ...
 *
 * Applies the `harmonica_footer_widget_areas` filter.
 *
 * @since 0.3.4
 *
 * @uses harmonica_structural_wrap() Optionally adds wrap with footer-widgets context.
 *
 * @return null Return early if number of widget areas could not be determined, or nothing is added to the first widget area.
 */
function harmonica_footer_widget_areas() {
	...
	echo apply_filters( 'harmonica_footer_widget_areas', $output, $footer_widgets );
