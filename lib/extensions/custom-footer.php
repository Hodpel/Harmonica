...
add_action( 'customize_register', 'harmonica_load_footer_customize_controls', 1 );

/* Register custom sections, settings, and controls. */
add_action( 'customize_register', 'harmonica_customize_footer_register' );

/* Add the footer content Ajax to the correct hooks. */
add_action( 'wp_ajax_harmonica_customize_footer_content', 'harmonica_customize_footer_content_ajax' );
add_action( 'wp_ajax_nopriv_harmonica_customize_footer_content', 'harmonica_customize_footer_content_ajax' );
...
if ( !function_exists('harmonica_load_footer_customize_controls') ) {
	function harmonica_load_footer_customize_controls() {

		class harmonica_Footer_Textarea extends WP_Customize_Control {
 ...
 */
if ( !function_exists('harmonica_customize_footer_register') ) {
	function harmonica_customize_footer_register( $wp_customize ) {

		/* Add the section. */
		$wp_customize->add_section(
			'footer',
			array(
				'title'      => esc_html__( 'Footer', 'harmonica' ),
				'priority'   => 149,
		...
		$wp_customize->add_setting(
			"custom_footer",
			array(
				'default'              => apply_filters( 'harmonica_footer_insert', '' ),
				'type'                 => 'theme_mod',
				'capability'           => 'edit_theme_options',
				'sanitize_callback'    => 'harmonica_customize_sanitize',
		...
		if ( $wp_customize->is_preview() && !is_admin() )
			add_action( 'wp_footer', 'harmonica_customize_preview_footer_script', 21 );
 ...
 */
if ( !function_exists('harmonica_customize_sanitize') ) {
	function harmonica_customize_sanitize( $setting, $object ) {

		/* Make sure we kill evil scripts from users without the 'unfiltered_html' cap. */
		if ( "harmonica_theme_settings[footer_insert]" == $object->id && !current_user_can( 'unfiltered_html' )  )
			$setting = stripslashes( wp_filter_post_kses( addslashes( $setting ) ) );

		/* Return the sanitized setting and apply filters. */
		return apply_filters( "harmonica_customize_sanitize", $setting, $object );
	}
}
 ...
 * @access private
 */
if ( !function_exists('harmonica_customize_footer_content_ajax') ) {
	function harmonica_customize_footer_content_ajax() {

		/* Check the AJAX nonce to make sure this is a valid request. */
		check_ajax_referer( 'harmonica_customize_footer_content_nonce' );
 ...
 * @access private
 */
if ( !function_exists('harmonica_customize_preview_footer_script') ) {
	function harmonica_customize_preview_footer_script() {

		/* Create a nonce for the Ajax. */
		$nonce = wp_create_nonce( 'harmonica_customize_footer_content_nonce' );
				...
				value.bind(
					function( to ) {
						jQuery.post( 
							...
							{ 
								action: 'harmonica_customize_footer_content',
