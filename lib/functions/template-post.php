 ...
 * @return bool
 */
function harmonica_post_has_content( $id = 0 ) {
	$post = get_post( $id );
 ...
 * @return void
 */
function harmonica_post_format_link() {
	echo harmonica_get_post_format_link();
}
 ...
 * @return string
 */
function harmonica_get_post_format_link() {
 ...
 * @return void
 */
function harmonica_post_terms( $args = array() ) {
	echo harmonica_get_post_terms( $args );
}
 ...
 * @return string
 */
function harmonica_get_post_terms( $args = array() ) {
	...
	$defaults = array(
		...
		'sep'        => _x( ', ', 'taxonomy terms separator', 'harmonica' )
	);
	...
	if ( !empty( $terms ) ) {
		$html .= $args['before'];
		$html .= sprintf( $args['items_wrap'], harmonica_get_attr( 'entry-terms', $args['taxonomy'] ), sprintf( $args['text'], $terms ) );
		$html .= $args['after'];
	}
 ...
 *
 * Output passes through 'harmonica_post_comments_shortcode' filter before returning.
 *
 * @since 0.9.0
 *
 ...
 */
function harmonica_post_comments( $atts = array() ) {

	$defaults = array(
		'after'       => '',
		'before'      => '',
		'hide_if_off' => 'enabled',
		'more'        => __( '% Comments', 'harmonica' ),
		'one'         => __( '1 Comment', 'harmonica' ),
		'zero'        => __( 'Leave a Comment', 'harmonica' ),
	);
	...
	return apply_filters( 'harmonica_post_comments_shortcode', $output, $atts );
 ...
 * harmonica_get_gallery_image_count() function since it will count all gallery items regardless of mime type.
 *
 ...
 * @return int
 */
function harmonica_get_gallery_item_count() {
 ...
 * @return int
 */
function harmonica_get_gallery_image_count() {
 ...
 * @return string
 */
function harmonica_get_content_url( $content ) {
 ...
 * @return string
 */
function harmonica_get_the_post_format_url( $url = '', $post = null ) {

	if ( empty( $url ) ) {

		$post = is_null( $post ) ? get_post() : $post;

		$content_url = harmonica_get_content_url( $post->post_content );
