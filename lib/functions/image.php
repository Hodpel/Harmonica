 ...
 */
function harmonica_get_image_id( $index = 0 ) {
 ...
 */
function harmonica_get_image( $args = array() ) {

	global $post;

	$defaults = apply_filters( 'harmonica_get_image_default_args', array(
		'format'   => 'html',
		'size'     => 'full',
		'num'      => 0,
		'attr'     => '',
	...
	$pre = apply_filters( 'harmonica_pre_get_image', false, $args, $post );
	if ( false !== $pre )
		return $pre;
	...
	elseif ( 'first-attached' == $args['fallback'] ) {
		$id = harmonica_get_image_id( $args['num'] );
		$html = wp_get_attachment_image( $id, $args['size'], false, $args['attr'] );
		list( $url ) = wp_get_attachment_image_src( $id, $args['size'], false, $args['attr'] );
	}
...
//		$output = false;
	...
	return apply_filters( 'harmonica_get_image', $output, $args, $id, $html, $url, $src );
}
 ...
 * @since 0.1.0
 *
 * @uses harmonica_get_image()
 *
 * @param array|string $args Optional. Image query arguments. Default is empty array
 * @return false Returns false if URL is empty
 */
function harmonica_image( $args = array() ) {

	$image = harmonica_get_image( $args );
 ...
 */
function harmonica_get_additional_image_sizes() {
 ...
 * @since 1.0.2
 *
 * @uses harmonica_get_additional_image_sizes()
 *
 * @return array Two-dimensional, with width, height and crop sub-keys
 */
function harmonica_get_image_sizes() {
	...
	$additional_sizes = harmonica_get_additional_image_sizes();
