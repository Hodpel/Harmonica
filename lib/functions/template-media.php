 ...
 * @return bool
 */
function harmonica_attachment_is_audio( $post_id = 0 ) {
 ...
 * @return bool
 */
function harmonica_attachment_is_video( $post_id = 0 ) {
 ...
 * @return int
 */
function harmonica_get_attachment_id_from_url( $url ) {
	global $wpdb;
 ...
 * @return string
 */
function harmonica_get_image_size_links() {
	...
	foreach ( $sizes as $size ) {
		...
		if ( !empty( $image ) && ( true === $image[3] || 'full' == $size ) ) {
			...
			$label = sprintf( __( '%1$s &#215; %2$s', 'harmonica' ), number_format_i18n( absint( $image[1] ) ), number_format_i18n( absint( $image[2] ) ) );
 ...
 * @return string
 */
function harmonica_get_audio_transcript( $post_id = 0 ) {
	...
	return apply_filters( 'harmonica_audio_transcript', $lyrics );
}
...
function harmonica_attachment() {
	...
	foreach ( $mime_type as $type ) {
		if ( function_exists( "harmonica_{$type}_attachment" ) )
			$attachment = call_user_func( "harmonica_{$type}_attachment", $mime, $file );

		$attachment = apply_filters( "harmonica_{$type}_attachment", $attachment );
	}

	echo apply_filters( 'harmonica_attachment', $attachment );
}
 ...
 * @return string
 */
function harmonica_application_attachment( $mime = '', $file = '' ) {
 ...
 * @return string
 */
function harmonica_text_attachment( $mime = '', $file = '' ) {
	$embed_defaults = wp_embed_defaults();
 ...
 * @return string
 */
function harmonica_audio_attachment( $mime = '', $file = '' ) {
 ...
 * @return string
 */
function harmonica_video_attachment( $mime = '', $file = '' ) {
