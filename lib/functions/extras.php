 ...
 * @since harmonica 9.0
 */
function harmonica_widget_classes($params) {
...
add_filter('dynamic_sidebar_params','harmonica_widget_classes');
 ...
 * @since harmonica 1.0
 */
function harmonica_first_and_last_menu_class($items) {
    $items[1]->classes[] = 'first';
    $items[count($items)]->classes[] = 'last';
    return $items;
}
add_filter('wp_nav_menu_objects', 'harmonica_first_and_last_menu_class');

/**
 * Get our wp_nav_menu() fallback, wp_page_menu(), to show a home link.
 */
function harmonica_page_menu_args( $args ) {
	$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'harmonica_page_menu_args' );

/**
 * Adds custom classes to the array of body classes.
 */
function harmonica_body_classes( $classes ) {
 ...
 */
function harmonica_default_menu( $args = array() ) {
	$defaults = array('sort_column' => 'menu_order, post_title', 'menu_class' => 'menu', 'echo' => true, 'link_before' => '', 'link_after' => '');
	$args = wp_parse_args( $args, $defaults );
	$args = apply_filters( 'harmonica_default_menu_args', $args );
	...
	$menu = apply_filters( 'harmonica_default_menu', $menu, $args );
	if ( $args['echo'] )
		echo $menu;
	else
		return $menu;
 ...
 */
function harmonica_truncate_phrase( $text, $max_characters ) {
...
function get_the_content_limit( $max_characters, $more_link_text = '(more...)', $stripteaser = false ) {
	...
	if ($max_characters < strlen( $content )) {
		$content = harmonica_truncate_phrase( $content, $max_characters );
		$no_more = false;
	} else {
		$no_more = true;
	}
...
if ( !get_theme_mod( 'more_scroll', 1 )) {
	add_filter( 'the_content_more_link', 'harmonica_remove_more_link_scroll' );
}

function harmonica_remove_more_link_scroll( $link ) {
