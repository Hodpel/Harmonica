...
add_action( 'init', 'harmonica_add_post_type_support' );

/* Filters the title for untitled posts. */
add_filter( 'the_title', 'harmonica_untitled_post' );
 ...
 * @return void
 */
function harmonica_add_post_type_support() {
 ...
 */
function harmonica_has_post_template( $template = '' ) {
 ...
 * @return string
 */
function harmonica_untitled_post( $title ) {

	if ( empty( $title ) && !is_singular() && in_the_loop() && !is_admin() )
		$title = __( '(Untitled)', 'harmonica' );
 ...
 * @return string
 */
function harmonica_locate_theme_file( $file_names ) {
 ...
 * @return array
 */
function harmonica_hex_to_rgb( $hex ) {
