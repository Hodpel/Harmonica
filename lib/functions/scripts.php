 ...
 * 'harmonica-scripts' feature to allow the framework to handle loading the stylesheets into 
 * the theme header or footer at an appropriate time.
 */

/* Register harmonica Core scripts. */
add_action( 'wp_enqueue_scripts', 'harmonica_register_scripts', 1 );

/* Load harmonica Core scripts. */
add_action( 'wp_enqueue_scripts', 'harmonica_enqueue_scripts' );
 ...
 * @return void
 */
function harmonica_register_scripts() {

	/* Supported JavaScript. */
	$supports = get_theme_support( 'harmonica-scripts' );
	...
	if ( isset( $supports[0] ) && in_array( 'mobile-toggle', $supports[0] ) )
		wp_register_script( 'mobile-toggle', esc_url( trailingslashit( harmonica_JS ) . "mobile-toggle{$suffix}.js" ), array( 'jquery' ), '20130528', true );
}
 ...
 * @return void
 */
function harmonica_enqueue_scripts() {
