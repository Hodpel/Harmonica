<?php
/**
 * The core functions file for the harmonica framework. Functions defined here are generally used across the 
 ...
 *
 * An example of a basic hook would be 'harmonica_header'.  The do_atomic() function extends that to 
 * give extra hooks such as 'harmonica_singular_header', 'harmonica_singular-post_header', and 
 * 'harmonica_singular-post-ID_header'.
 *
 ...
 */
function harmonica_do_atomic( $tag = '', $arg = '' ) {
	...
	do_action_ref_array( "harmonica_{$tag}", $args );

	/* Loop through context array and fire actions on a contextual scale. */
	foreach ( (array) harmonica_get_context() as $context )
		do_action_ref_array( "harmonica_{$context}_{$tag}", $args );
}
 ...
 * without having to know how to use WordPress conditional tags.  The theme handles the logic.
 *
 * An example of a basic hook would be 'harmonica_entry_meta'.  The apply_atomic() function extends 
 * that to give extra hooks such as 'harmonica_singular_entry_meta', 'harmonica_singular-post_entry_meta', 
 * and 'harmonica_singular-post-ID_entry_meta'.
 *
 * @since  0.9.0
 * @access public
 * @param  string $tag     Usually the location of the hook but defines what the base hook is.
 ...
 */
function harmonica_apply_atomic( $tag = '', $value = '' ) {
	...
	$value = $args[0] = apply_filters_ref_array( "harmonica_{$tag}", $args );

	/* Loop through context array and apply filters on a contextual scale. */
	foreach ( (array) harmonica_get_context() as $context )
		$value = $args[0] = apply_filters_ref_array( "harmonica_{$context}_{$tag}", $args );
...
/**
 * Wraps the output of harmonica_apply_atomic() in a call to do_shortcode(). This allows developers to use 
 ...
 * @return mixed  $value The value after it has been filtered.
 */
function harmonica_apply_atomic_shortcode( $tag = '', $value = '' ) {
	return do_shortcode( harmonica_apply_atomic( $tag, $value ) );
}
 ...
 */
function harmonica_format_hook( $tag, $context = '' ) {
	return "harmonica" . ( ( !empty( $context ) ) ? "_{$context}" : "" ). "_{$tag}";
}
 ...
 */
function harmonica_set_content_width( $width = '' ) {
 ...
 */
function harmonica_get_content_width() {
	global $content_width;
