 ...
 * @return void
 */
function harmonica_comment_reply_link( $args = array() ) {
	echo harmonica_get_comment_reply_link( $args );
}
 ...
 * @return string
 */
function harmonica_get_comment_reply_link( $args = array() ) {
 ...
 */
function harmonica_list_comments_args() {
	...
	return apply_filters( 'harmonica_list_comments_args', $args );
}
 ...
 * comment.php.
 *
 * The templates are saved in $harmonica->comment_template[$comment_type], so each comment template
 ...
 * @return void
 */
function harmonica_comments_callback( $comment, $args, $depth ) {
	global $harmonica;
	...
	if ( !isset( $harmonica->comment_template) || !is_array( $harmonica->comment_template ) )
		$harmonica->comment_template = array();

	/* Check if a template has been provided for the specific comment type.  If not, get the template. */
	if ( !isset( $harmonica->comment_template[$comment_type] ) ) {
		...
		$templates = apply_filters( 'harmonica_comment_template_hierarchy', $templates, $comment_type );

		/* Locate the comment template. */
		$template = locate_template( $templates );

		/* Set the template in the comment template array. */
		$harmonica->comment_template[ $comment_type ] = $template;
	}

	/* If a template was found, load the template. */
	if ( !empty( $harmonica->comment_template[ $comment_type ] ) )
		require( $harmonica->comment_template[ $comment_type ] );
}

/**
 * Ends the display of individual comments. Uses the callback parameter for wp_list_comments(). 
 * Needs to be used in conjunction with harmonica_comments_callback(). Not needed but used just in 
 ...
 * @return void
 */
function harmonica_comments_end_callback() {
	echo '</li><!-- .comment -->';
}
