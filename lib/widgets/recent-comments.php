...
class harmonica_recent_comments extends WP_Widget {

	function __construct() {
        $widget_ops = array( 'classname' => 'widget_harmonica_recent_comments', 'description' => __('Displays recent comments with user avatars.', 'harmonica') );
        parent::__construct( 'widget_harmonica_recent_comments', __('Recent Comments','harmonica'), $widget_ops );
    }
					...
					if ( $comments ) {
						foreach ( $comments as $comment ) { ?>
							...
							<li>
								...
								<a href="<?php echo get_permalink($comment->comment_post_ID); ?>#comment-<?php echo $comment->comment_ID; ?>" title="<?php _e('Comment to','harmonica'); echo ' ' . get_the_title($comment->comment_post_ID) . ', '; _e('posted','harmonica'); echo ' ' . get_the_time(get_option('date_format')); ?>">
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('number_of_comments'); ?>"><?php _e('Number of comments to display', 'harmonica'); ?>:
			...
			<small>(<?php _e('Defaults to 5 if empty','harmonica'); ?>)</small>
		</p>
				
		<?php
	}
}
register_widget('harmonica_recent_comments'); ?>