...
add_action( 'admin_menu', 'harmonica_admin_setup' );
 ...
 * @return void
 */
function harmonica_admin_setup() {

	/* Registers admin stylesheets for the framework. */
	add_action( 'admin_enqueue_scripts', 'harmonica_admin_register_styles', 1 );

	/* Loads admin stylesheets for the framework. */
	add_action( 'admin_enqueue_scripts', 'harmonica_admin_enqueue_styles' );
}
 ...
 * @return void
 */
function harmonica_admin_register_styles() {

	/* Use the .min stylesheet if SCRIPT_DEBUG is turned off. */
	$suffix = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '' : '.min';

	wp_register_style( 'harmonica-admin', trailingslashit( harmonica_CSS ) . "admin.css" );

	wp_register_style( 'harmonica-admin-widgets', trailingslashit( harmonica_CSS ) . "admin-widgets{$suffix}.css" );

	wp_register_script( 'harmonica-admin', esc_url( trailingslashit( harmonica_JS ) . "admin.js" ), array( 'jquery' ), '20130528', true );
}
 ...
 * @return void
 */
function harmonica_admin_enqueue_styles( $hook_suffix ) {

	wp_enqueue_style( 'harmonica-admin' );

	if ( current_theme_supports( 'harmonica-widgets' ) && 'widgets.php' == $hook_suffix )
		wp_enqueue_style( 'harmonica-admin-widgets' );	   
 ...
 */
function harmonica_get_post_templates( $post_type = 'post' ) {
	global $harmonica;

	/* If templates have already been called, just return them. */
	if ( !empty( $harmonica->post_templates ) && isset( $harmonica->post_templates[ $post_type ] ) )
		return $harmonica->post_templates[ $post_type ];

	/* Else, set up an empty array to house the templates. */
	else
		$harmonica->post_templates = array();
	...
	/* Add the templates to the global $harmonica object. */
	$harmonica->post_templates[ $post_type ] = array_flip( $post_templates );
